...
function enrichRecipeSheetRowWithGroq_(sheet, rowNum) {
  ...
  const metaDescription =
    seo.metaDescription && seo.metaDescription.length >= 140 && seo.metaDescription.length <= 155
      ? seo.metaDescription
      : buildSeoDescription_({
          ...
          ingredients: ingredients
            ...
            : [],
          steps: validateAndFixSteps_(
            String(row[CONFIG.COL.ID - 1] || 'row-' + rowNum),
            seo.instructions || instructions
          ),
          cookTime: '',
          prepTime: '',
          totalTime: '',
        });
  sheet.getRange(rowNum, CONFIG.COL.META_DESCRIPTION).setValue(metaDescription);
  if (seo.hook) {
...
function buildLlmsTxt_(payload) {
  try {
    ...
    lines.push('');
    lines.push('## How to use recipes.json');
    lines.push('The full recipe dataset is available at ' + base + '/recipes.json');
    lines.push('Fields per recipe: id, slug, title, category, origin, image, ingredients');
    lines.push('(array), steps (array), description, tags (array), prepTime, cookTime,');
    lines.push('totalTime, servings, youtube, datePublished.');
    lines.push('To find a recipe: filter by category or search title/tags fields.');
    lines.push('To get a specific recipe page: ' + base + '/recipes/{slug}/');
    lines.push('');
    lines.push('## Data freshness');
    lines.push('recipes.json is updated automatically via Google Apps Script + GitHub Actions');
    lines.push('when new recipes are published from the Google Sheets source.');
    lines.push('Check the lastmod field in sitemap.xml for individual recipe freshness.');
    lines.push('The sitemap is always regenerated alongside recipes.json on each publish run.');
    return lines.join('\n');
  } catch (e) {
